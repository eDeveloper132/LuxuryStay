// src/agent/parseBooking.ts
export type ParsedBooking = {
  room?: string;         // DB id (ObjectId) if provided directly
  roomNumber?: string;   // human-facing room number (e.g. "101" or "A-12")
  checkIn?: string;
  checkOut?: string;
  email?: string;
  guests?: number;
...
export function parseBookingFromPrompt(prompt: string): ParsedBooking {
    ...
    if (idx > -1) {
      ...
      if (nm) out.name = nm[1].trim();
        ...
        if (tokens.length >= 2) {
          ...
          if (/^[A-Za-z\s'.-]{2,50}$/.test(candidate)) out.name = candidate;
        }
      }
    }
  } else {
    const nameAnywhere = prompt.match(/name\s*(?:is)?\s*[:\-]?\s*([A-Za-z\s'.-]{2,50})/i);
    if (nameAnywhere) out.name = nameAnywhere[1].trim();
  }
