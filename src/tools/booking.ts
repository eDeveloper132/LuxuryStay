...
function parseDateString(s?: unknown): Date | null {
  if (!s) return null;
  const d = new Date(String(s));
  return isNaN(d.getTime()) ? null : d;
}

export const bookRoomHandler: ToolHandler = async (args, context) => {
  try {
    const room = (args.room ?? args.roomId ?? '') as string;
    const checkInRaw = args.checkIn;
    const checkOutRaw = args.checkOut;
    const guestName = (args.guestName ?? '') as string;
    const guestId = (args.guestId ?? context?.userId ?? '') as string;
    const guests = Number(args.guests ?? 1);

    // Basic validation
    if (!room) return 'Error: `room` (room id) is required. Provide room id or roomNumber.';
    const checkIn = parseDateString(checkInRaw);
    const checkOut = parseDateString(checkOutRaw);
    if (!checkIn || !checkOut) return 'Error: invalid or missing checkIn/checkOut dates.';
    if (checkIn >= checkOut) return 'Error: checkOut must be after checkIn.';
    const now = new Date();
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const checkInStart = new Date(checkIn.getFullYear(), checkIn.getMonth(), checkIn.getDate());
    if (checkInStart <= todayStart) return 'Error: Check-in date must be in the future.';
...
export const cancelBookingHandler: ToolHandler = async (args, context) => {
  try {
    const bookingId = String(args.bookingId ?? '');
    const reason = args.reason ? String(args.reason) : undefined;
    const requesterId = (args.requesterId ?? context?.userId) as string;
    const requesterRole = (args.requesterRole ?? context?.role) as string | undefined;
    ...
    if (booking.status === 'cancelled') return `Error: Booking ${bookingId} already cancelled.`;
    ...
    if (reason) (booking as any).cancelReason = reason;
    await booking.save();
