...
function parseDateStrict(d: unknown) {
...
async function isRoomAvailable(roomId: string, checkIn: Date, checkOut: Date) {
...
export const getUserByEmailDirect: ToolHandler = async (args, ctx: Context = {}) => {
  try {
    ...
    if (!user) return JSON.stringify({ ok: false, status: 404, error: 'not_found' });
    return JSON.stringify({ ok: true, user }, null, 2);
  } catch (err: any) {
    return JSON.stringify({ ok: false, error: String(err.message ?? err) });
  }
};
export const getRoomByNumberDirect: ToolHandler = async (args, ctx: Context = {}) => {
  try {
    const roomNumber = String(args.roomNumber ?? args.number ?? '').trim();
    if (!roomNumber) return JSON.stringify({ ok: false, error: 'roomNumber_required' });

    const room = await RoomModel.findOne({ number: roomNumber }).lean();
    if (!room) return JSON.stringify({ ok: false, status: 404, error: 'not_found' });
...
export const getRoomsDirect: ToolHandler = async (args, ctx: Context = {}) => {
  try {
    const filter: any = {};
    ...
    if (checkIn && checkOut) {
      ...
      for (const r of rooms) {
        ...
        if (avail) availableRooms.push(r);
      }
...
export const bookRoomDirect: ToolHandler = async (args, ctx: Context = {}) => {
  ...
  try {
    ...
    if (!room) return JSON.stringify({ ok: false, error: 'room_not_found' });

    // check availability
    const available = await isRoomAvailable(roomId, checkIn, checkOut);
    if (!available) return JSON.stringify({ ok: false, error: 'room_unavailable' });

    // resolve user/guest
    let guestId: any = null;
    if (email) {
      const user = await UserModel.findOne({ email }).lean();
      if (!user) return JSON.stringify({ ok: false, error: 'user_not_found_for_email' });
      guestId = user._id;
    } else if (ctx.userId) {
      guestId = ctx.userId;
    } else {
...
export const cancelBookingDirect: ToolHandler = async (args, ctx: Context = {}) => {
  try {
    const bookingId = String(args.bookingId ?? args.id ?? '').trim();
    let emailSent = false;
    let emailError: string | null = null;
    if (!bookingId) return JSON.stringify({ ok: false, error: 'bookingId_required' });

    const booking = await BookingModel.findById(bookingId);
    if (!booking) return JSON.stringify({ ok: false, error: 'booking_not_found' });
