...
function buildAuthHeaders(ctx) {
    ...
    if (!ctx.cookie && ctx.userId)
        headers['x-user-id'] = ctx.userId;
    if (!ctx.cookie && ctx.role)
        headers['x-user-role'] = ctx.role;
    return headers;
}
/**
...
async function fetchUserByEmail(email, ctx) {
    try {
 ...
 */
export const bookRoomViaApi = async (args, context = {}) => {
    try {
        ...
        const headers = buildAuthHeaders(context);
        const body = { room, checkIn, checkOut };
        if (guests)
            body.guests = guests;
        // If an email is provided, try to resolve user and attach guest/user id
        if (email) {
            const userRes = await fetchUserByEmail(email, context);
            if (!userRes.ok) {
 ...
 */
export const cancelBookingViaApi = async (args, context = {}) => {
    try {
 ...
 * GET rooms list
 */
export const getRoomsViaApi = async (args, context = {}) => {
    try {
        const headers = buildAuthHeaders(context);
        const params = new URLSearchParams();
        if (args.type)
            params.append('type', String(args.type));
        if (args.capacity)
            params.append('capacity', String(args.capacity));
 ...
 */
export const getUserByEmailViaApi = async (args, context = {}) => {
    try {
        const email = String(args.email ?? '').trim();
        if (!email)
            return 'Error: email is required';
        const res = await fetchUserByEmail(email, context);
