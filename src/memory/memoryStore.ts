...
export function redactPII(text: string) {
  ...
  t = t.replace(/(\+?\d{1,3}[-.\s]?)?(\(?\d{2,4}\)?[-.\s]?)?\d{3,4}[-.\s]?\d{3,4}/g, (m) => {
    if (/^\d{4}$/.test(m.trim())) return m; // allow year-like short numbers
...
export async function embedText(text: string) {
  ...
  if (!vec) throw new Error('No embedding returned from OpenAI');
 ...
 */
export async function upsertConversation(email: string, role: 'user' | 'assistant' | 'tool' | 'system', text: string) {
  try {
    ...
    const vectors: any[] = embeddings.map((vec, i) => ({
      id: uuidv4(),
      values: vec,
      metadata: {
        emailHash,
 ...
 */
export async function fetchRelevantMemory(email: string, query: string, topK = MEMORY_TOP_K) {
  try {
    if (!email) return [];
    await initClientsIfNeeded();
