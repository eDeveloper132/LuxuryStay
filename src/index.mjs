 ...
 */
function makeSessionKey(context, rawCookie, ip) {
    if (context?.userId)
        return `user:${String(context.userId)}`;
    if (rawCookie)
...
function safeParseToolOutput(raw) {
    if (raw == null)
        return null;
    if (typeof raw === 'string') {
        try {
...
async function safeExtractBooking(obj) {
...
function sanitizeRoomsList(rooms) {
    ...
    try {
        ...
        if (wantsCancel) {
            ...
            if (bookingIdMatch) {
                ...
                const bookingQuery = { room: roomDoc._id, status: { $ne: 'cancelled' } };
                if (parsed.email) {
                    const u = await UserModel.findOne({ email: parsed.email.toLowerCase() }).lean();
                    if (u)
                        bookingQuery.guest = u._id;
                }
                else if (context.userId) {
                    bookingQuery.guest = context.userId;
                }
                if (parsed.checkIn)
                    bookingQuery.checkIn = new Date(parsed.checkIn);
                if (parsed.checkOut)
                    bookingQuery.checkOut = new Date(parsed.checkOut);
            ...
            try {
                pendingBookings.delete(sessionKey);
            }
            catch { }
            if (safeBooking) {
        ...
        if (wantsBooking && !parsed.room && !parsed.roomNumber) {
            ...
            const roomArgs = {};
            if (parsed.checkIn)
                roomArgs.checkIn = parsed.checkIn;
            if (parsed.checkOut)
                roomArgs.checkOut = parsed.checkOut;
            if (req.body.type)
                roomArgs.type = req.body.type;
            ...
            let roomsToShow = [];
            if (Array.isArray(roomsObj?.rooms))
                roomsToShow = roomsObj.rooms;
            else if (Array.isArray(roomsObj))
                roomsToShow = roomsObj;
            else if (roomsObj && typeof roomsObj === 'object' && (roomsObj._id || roomsObj.number))
                roomsToShow = [roomsObj];
            ...
            const listText = safeRooms.slice(0, 20).map((r, idx) => {
        ...
        if (parsed.roomNumber) {
            ...
            if (bookingContext.guests)
                bookingArgs.guests = bookingContext.guests;
            if (bookingContext.email)
                bookingArgs.email = bookingContext.email;
